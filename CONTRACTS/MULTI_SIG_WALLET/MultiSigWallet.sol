...
import "../SHARED_CONTRACTS/ERC20TransferInterface.sol";
...
contract MultiSigWallet is Ownable {
    ...
    * 
    * @param _authorisedAddrs The list of addresses which will be authorised to request transactions and
    * confirm transactions (ETH and ERC20)
    **/
    function MultiSigWallet(address[] _authorisedAddrs) public {
        require(_authorisedAddrs.length >= 2);
        for(uint i = 0; i < _authorisedAddrs.length; i++) {
            require(!isAuthorised[_authorisedAddrs[i]]);
            isAuthorised[_authorisedAddrs[i]] = true;
        }
